...
def build_discord_message(stage: str, platform_name: str, custom_message: str = None) -> dict:
    ...
    """
    if custom_message:
        return {"content": render_notification_template(custom_message, stage, platform_name),
...
def build_telegram_message(stage: str, platform_name: str, custom_message: str = None) -> str:
    ...
    """
    if custom_message:
        return render_notification_template(custom_message, stage, platform_name)
...
CONST_ACTION_SPEED_MIN = 0.0
CONST_ACTION_SPEED_MAX = 5.0
CONST_ACTION_SPEED_DEFAULT = 1.0


def get_action_speed_multiplier(config_dict=None):
    """Read the global action-speed multiplier.

    1.0 = unchanged, 0.5 = every humanised wait halved, 0 = no waiting at all,
    2.0 = twice as patient. Clamped, and any malformed value falls back to 1.0 so a bad
    setting can never stall the bot or make it hammer the site.
    """
    if not config_dict:
        return CONST_ACTION_SPEED_DEFAULT
    try:
        raw = config_dict.get("advanced", {}).get("action_speed_multiplier", CONST_ACTION_SPEED_DEFAULT)
    except Exception:
        return CONST_ACTION_SPEED_DEFAULT
    ...
    if value != value:  # NaN
        return CONST_ACTION_SPEED_DEFAULT
    return max(CONST_ACTION_SPEED_MIN, min(CONST_ACTION_SPEED_MAX, value))


def scale_humanized_delay(min_s, max_s, config_dict=None):
    """Pick a random delay in [min_s, max_s] and scale it by the multiplier.

    Returns seconds (float). Kept separate from the sleep itself so it can be unit-tested
    without an event loop, and so callers that need the number (not the wait) can reuse it.
    """
    multiplier = get_action_speed_multiplier(config_dict)
    if max_s < min_s:
...
CONST_NOTIFY_STAGE_LABEL = {
    "ticket": "找到票",
    "order": "搶到票",
}


def render_notification_template(template: str, stage: str, platform_name: str) -> str:
    """Expand {placeholders} in a user-supplied notification message.

    Supported: {time} {date} {datetime} {timestamp} {platform} {stage} {stage_en}
    The timestamp is taken at send time, which is the moment the bot reached the order
    page - that is what the user means by "the time I got the ticket".

    An unknown placeholder is left verbatim rather than raising, so a typo degrades to a
    slightly odd message instead of losing the notification entirely.
    """
    if not template:
        return template
    now = datetime.now()
    values = {
        "time": now.strftime("%H:%M:%S.") + ("%03d" % (now.microsecond // 1000)),
        "date": now.strftime("%Y/%m/%d"),
        "datetime": now.strftime("%Y/%m/%d %H:%M:%S.") + ("%03d" % (now.microsecond // 1000)),
        "timestamp": str(int(now.timestamp())),
        "platform": platform_name or "Unknown",
        "stage": CONST_NOTIFY_STAGE_LABEL.get(stage, stage or ""),
        "stage_en": stage or "",
    }
    ...
    try:
        ...
        # Unbalanced braces in the user's text -> send it as-is rather than failing.
        return template
