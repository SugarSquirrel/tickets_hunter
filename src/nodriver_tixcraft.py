...
def parse_refresh_datetime(target_str):
    ...
    # Supports, with or without fractional seconds (up to milliseconds):
    #   "YYYY/MM/DD HH:MM:SS[.fff]" | "HH:MM:SS[.fff]" (today) | "" (disabled)
    # Sub-second precision matters here: on-sale moments are announced to the second, so
    # aiming at e.g. 09:59:59.700 is a real strategy, and truncating it to whole seconds
    # silently threw that away.
    # Returns: datetime or None
    if not target_str or not target_str.strip():
        return None
    target_str = target_str.strip()
    # Accept a comma as the decimal separator too (some locales type it that way).
    normalized = target_str.replace(',', '.')
    if '/' in normalized:
