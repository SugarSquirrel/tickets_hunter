...
<div class="mb-0">
  ...
  <button type="button" class="btn btn-outline-secondary btn-sm me-2 mb-2" onclick="fillHomepage('https://www.ticketek.com.au')">Ticketek Australia</button>
    ...
    applyOrRestore('#refresh_datetime', 'placeholder', 'YYYY/MM/DD HH:MM:SS.fff (ms optional)');
...
function renderAdvancedTabTranslations() {
    ...
    setRowLabelForField('action_speed_multiplier', fieldLabel('Action speed multiplier', 'action_speed_multiplier'));
...
function load_settins_to_form(settings)
{
    if (settings)
    {
        ...
        action_speed_multiplier.value = (settings.advanced.action_speed_multiplier === undefined ||
                                         settings.advanced.action_speed_multiplier === null)
            ? 1 : settings.advanced.action_speed_multiplier;
...
function save_changes_to_dict(silent_flag)
{
    ...
    if (!ticket_number_value)
    {
        ...
        // Empty means "leave it alone" -> 1.0. Clamp so a typo cannot make the bot
        // either freeze (huge value) or hammer the site (negative value).
        let _speed = parseFloat(action_speed_multiplier.value);
        if (isNaN(_speed)) { _speed = 1; }
        settings.advanced.action_speed_multiplier = Math.min(5, Math.max(0, _speed));
